...
1. **Nenhum rastro de máquina versionado.** Proibido: caminho absoluto de pasta pessoal de usuário Mac ou Linux (padrao-ouro:ignorar), ou `C:\Users\...`; `.DS_Store`; qualquer comando que só existe no macOS (`osascript`, `pbcopy`, `open` sem fallback). O validador (`tools/validar_plugins.py`) reprova esse tipo de caminho em qualquer arquivo versionado.
